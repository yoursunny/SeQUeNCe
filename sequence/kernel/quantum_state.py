...
class KetState(State):
    ...
    def __init__(self, amplitudes: list[complex], keys: list[int], truncation: int = 1):
        ...
        assert math.isclose(
            sum([abs(a) ** 2 for a in amplitudes]), 1), "Squared amplitudes do not sum to 1"
...
class DensityState(State):
    ...
    def __init__(self, state: list[list[complex]], keys: list[int], truncation: int = 1):
        ...
        # check formatting
        assert abs(trace(array(state)) -
                   1) < 0.01, "density matrix trace must be 1"
        for row in state:
...
class FreeQuantumState(State):
    ...
    def set_state(self, state: tuple[complex]):
        ...
        assert abs(sum([abs(a) ** 2 for a in state]) -
                   1) < 1e-5, "Squared amplitudes do not sum to 1"
...
class BellDiagonalState(State):
    ...
    def __init__(self, diag_elems: list[float], keys: list[int]):
        ...
        assert len(
            keys) == 2, "BellDiagonalState density matrix are only supported for 2-qubit entangled states."
